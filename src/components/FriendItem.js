...
function FriendItem({ friend, isSelected, onSelect, onDeselect }) {
  const handleClick = () => {
    isSelected ? onDeselect() : onSelect(friend);
  };

  const handleCompare = () => {
    return friend.balance > 0 ? "owes" : friend.balance < 0 ? "owe" : "even";
  };
